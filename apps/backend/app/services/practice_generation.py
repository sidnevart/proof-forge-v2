...
CONSPECT_SYSTEM_PROMPT = (
    ...
    "- Пиши в Markdown. Используй заголовки ##/###, списки, **жирный** для терминов.\n"
    "- Для любого процесса, потока данных, архитектуры или жизненного цикла рисуй "
    "диаграмму Mermaid в fenced-блоке ```mermaid (синтаксис flowchart TD / sequenceDiagram). "
    "Минимум 1–2 диаграммы на конспект.\n"
    "- Для сравнений (подходы, варианты, trade-offs) используй Markdown-таблицу.\n"
    "- Примеры кода — в fenced-блоках с указанием языка (```python, ```ts и т.п.).\n"
...
«на пальцах», не экономь на примерах и схемах. Используй структуру:

## Обзор
[3-4 предложения: что это, зачем нужно и какую проблему решает]

**Аналогия на пальцах:** [бытовой образ, который объясняет суть в 1-2 предложениях]

```mermaid
flowchart TD
  [Схема общей картины темы — как элементы связаны между собой]
```

## Ключевые концепции
[Для КАЖДОЙ концепции: ### Название → подробное объяснение простыми словами →
«Аналогия на пальцах: …» → пример кода если уместно. Где есть процесс или поток —
добавь Mermaid-диаграмму. Раскрывай 3-5 концепций глубоко, а не списком.]

## Как это работает шаг за шагом
[Пошаговый разбор механизма/потока на конкретном примере. Если есть последовательность
взаимодействий — нарисуй ```mermaid sequenceDiagram. Покажи код и что происходит на каждом шаге.]

## Сравнение подходов
[Markdown-таблица: вариант / когда использовать / плюсы / минусы. Если сравнивать нечего —
таблица типичных параметров или режимов работы.]

## Практическое применение
[Реальные сценарии, типичные паттерны, как и где применяется на практике — с примерами]

## Типичные ошибки
[3-4 частых ошибки. Для каждой: почему возникает + пример неправильного кода (```) и
правильного (```) рядом, с объяснением разницы]

## Важно запомнить
[4-6 ключевых тезисов — коротко и ёмко]

Включай примеры кода и Mermaid-диаграммы там, где они реально помогают пониманию.
