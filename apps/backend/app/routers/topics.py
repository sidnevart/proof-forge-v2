...
from app.services.content_reduction import (
    ...
    MAX_MAP_CHUNKS as _MAX_MAP_CHUNKS,
...
# Chunking thresholds live in app.services.content_reduction (shared with conspect gen).
...
async def _extract_concepts_from_chunk(
    ...
    prompt = build_extract_prompt(topic_name, material_name, chunk)
...
async def _run_capsule_generation(
    ...
    try:
        ...
        # MAP calls are capped at _MAX_MAP_CHUNKS, so the timeout estimate is too.
        est_chunks = min(_MAX_MAP_CHUNKS, max(1, total_chars // _CHUNK_SIZE))
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            if use_chunked:
                planned, dropped = plan_chunks(
                    [(m["name"], m["content_text"]) for m in materials_snapshot],
                    _CHUNK_SIZE,
                    _MAX_MAP_CHUNKS,
                )
                ...
                for chunk_idx, (mat_name, chunk) in enumerate(planned, 1):
                    ...
                    extract, u = await _extract_concepts_from_chunk(
                        client, settings, topic_name, mat_name, chunk
                    )
