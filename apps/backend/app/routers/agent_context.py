...
async def get_agent_context(
    ...
    profile = await user_repo.get_profile(db, userId)
    profile_dict = {
    ...
    capsules = await capsule_repo.get_user_capsules(db, userId)
    capsules_list = [
        {"id": c.id, "topic_id": c.topic_id, "summary": c.summary, "created_at": c.created_at.isoformat()}
        for c in capsules[:5]
    ]

    weak_spots = await capsule_repo.get_user_weak_spots(db, userId)
    spots_list = [
        {"concept": ws.concept, "severity": ws.severity, "topic_id": ws.topic_id}
        for ws in weak_spots[:10]
    ]

    events = await event_repo.get_recent_events(db, userId)
    events_list = [
